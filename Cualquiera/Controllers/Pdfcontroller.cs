...
namespace Cualquiera.Controllers
{
    ...
    public class Pdfcontroller : Controller
    {
        ...
        [HttpPost]
        [Route("/Controllers/Pdfcontroller/GeneratePdf1")]
        public async Task<IActionResult> GeneratePdf1(string datos1)
        {

            var lista = JsonConvert.DeserializeObject<string[][]>(datos1);
            ...
            documento.Add(new Paragraph("Nombre__Apellidos___Fecha__________Rut________Email_Disponible"));
            for (int i = 0; i < lista.Length; i++)
            {
                string nom = lista[i][0].ToString().Trim();
                string appell = lista[i][1].ToString().Trim();
                ...
                string ru = lista[i][3].ToString().Trim();
                string em = lista[i][4].ToString().Trim();
                string dis = lista[i][5].ToString().Trim();

                documento.Add(new Paragraph($"{nom}__{appell}___{fechass}____{ru}_____{em}_{dis}"));
            }
